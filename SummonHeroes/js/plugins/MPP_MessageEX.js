...
/*:
 * @plugindesc 【ver.1.2】文章表示の機能を拡張したり表示の演出を追加します。
 * @author 木星ペンギン
 * @help [文章の表示]の制御文字:
...
Window_Message.prototype.convertEscapeCharacters = function(text) {
    var _convertEscapeCharacters = Alias.WiMe_convertEscapeCharacters ||
    ...
    text = text.replace(/\x1bSET\[(\d+)\]/gi, function() {
        ...
        return _convertEscapeCharacters.call(this, setText);
    }.bind(this));
...
Window_Message.prototype.calcTextHeight = function(textState, all) {
    ...
    var _calcTextHeight = Alias.WiMe_calcTextHeight ||
            Window_Base.prototype.calcTextHeight;
    return Math.max(_calcTextHeight.call(this, textState, all), maxFontSize + 8);
};
...
Window_MessageName.prototype.setName = function(name, colorIndex) {
    if (this._name !== name) {
        ...
        if ($gameMessage.positionType() === 0) {
            var y = this._messageWindow.y - MPPlugin.messageNamePos[1];
