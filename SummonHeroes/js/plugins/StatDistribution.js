...
Imported.StatDistribution = true;
...
Put the following tags to determine how much the param/stat will increase when spending:
...
Put the following tags to determine how many points need to be spend to increase the param/stat:
...
If using Yanfly Main Menu Manger:
Menu Name : PluginManager.parameters('StatDistribution')["Menu Name"]
Menu Symbol : stats
Menu Show : true
Menu Enabled : true
Menu Ext:
Menu Main Bind: this.commandPersonal.bind(this)
Menu Actor Bind:SceneManager.sceneDistribution(this.actor())
...
ChangeLog:
--1.3
-Added option to change point type, JP (Job Points) and GP (Gold) and XP (Experience)
-Added option to control how much param/stat is given in one spending.
-Added Menu Name for when Adding to Yanfly Main Menu Mangaer to Call on.
...
*********************************************************************************
@param Menu Name
@desc Name for the Menu if using Yanfly Main Menu Manager
@default Stats

@param Points Type
@desc Which points will be used in the game. JP(Job Points), DP(Default Points), GP(Gold Points aka Gold), XP(Expereince Points aka Experience)
@default DP
...
@param RestoreMPAndHP
@desc Retore HP And MP when increasing stats.
@default true
...
var pointsUsed = String(parameters["Points Type"]);
var restore = String(parameters["RestoreMPAndHP"]);
...
//Stat increase per stat points
Game_Actor.prototype.statPerPoints = function(id) {
	var str;
	switch(id) {
		case 0:
			str = "php";
...
//Xstat increase per stat point
Game_Actor.prototype.xstatPerPoints = function(id) {
	var str;
	switch(id) {
		case 0:
			str = "phit";
...
// JP Edits
    if (pointsUsed === "JP") {
		this.gainJp(amount, this.currentClass().id);	
	}
	else if (pointsUsed === "GP") {
		$gameParty.gainGold(amount);
	}
	else if (pointsUsed === "XP") {
		 var value = this.currentExp() + amount;
...
// JP Edits
    if (pointsUsed === "JP"){
	return this._jp[this.currentClass().id];
	}
	else if (pointsUsed === "GP"){
...
Window_Points.prototype.drawThings = function() {
	...
	var pNs = String(eval(pointsName));
    this.drawText(pNs, x, y, mWidth);
...
Scene_Distribution.prototype.refreshActor = function() {
    var actor = this.actor();
    this._statusWindow.setActor(actor);
    this._itemWindow.setActor(actor);
};

Scene_Distribution.prototype.user = function() {
    return this.actor();
};
Scene_Distribution.prototype.onActorChange = function() {
    Scene_Distribution;
};
...
Scene_Distribution.prototype.increaseParam = function(id) {
	var x = Input.isPressed('shift') ? shiftIncrease * this.actor().statPerPoints(id) : this.actor().statPerPoints(id);
	var amount;
	var condition = this.actor().canIncrease(id, x, this._params[id]);
	if ((this.actor().statPoints() >= x) && condition) {
		amount = this.actor().statPerPoint(id);
...
Scene_Distribution.prototype.increaseExParam = function(id) {
	var x = Input.isPressed('shift') ?  Input.isPressed('shift') ? shiftIncrease * this.actor().xstatPerPoints(id) : this.actor().xstatPerPoints(id);
	var amount;
	var condition = this.actor().canIncreasex(id, x, this._xparams[id]);
	if ((this.actor().statPoints() >= x) && condition) {
		amount = this.actor().xstatPerPoints(id);
...
Scene_Distribution.prototype.decreaseParam = function(id) {
	var x = Input.isPressed('shift') ? shiftIncrease * this.actor().statPerPoints(id) : this.actor().statPerPoints(id);
	var amount = this.actor().statPerPoint(id);
...
Scene_Distribution.prototype.decreaseExParam = function(id) {
	...
	var amount = this.actor().xstatPerPoint(id);
